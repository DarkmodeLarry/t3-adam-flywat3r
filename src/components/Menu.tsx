      ...
      <div className='mx-auto max-w-2xl py-16 px-4 sm:px-6 lg:max-w-7xl lg:px-8'>
        <div className='flex w-full justify-between'>
          <h2 className='text-2xl font-bold tracking-tight text-gray-900'>
            On our menu {format(parseISO(selectedTime), 'MMM do, yyyy')}
          </h2>
          <Select
            onChange={(e) => {
        ...
        <div className='mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8'>
          {filteredMenuItems?.map((menuItem) => (
            <div key={menuItems.id} className='group relative'>
              <div className='min-h-80 aspect-w-1 lg:aspect-none w-full overflow-hidden rounded-md bg-gray-200 group-hover:opacity-75 lg:h-80'>
